// DFIX:TODO
//
// FIX: scaling radar stuff
// FIX: bottom-left radar image artifacts
...
uses OpenGl, Windows, Classes, Controls, ExtCtrls, Graphics, SysUtils, StdCtrls, RequiredTypes,
  EditorItem, gtatxd, GTAText, Dialogs; //geometry
...
type
  ...
  private
    ...
    DoModels, DoBackground, DoTextures, DoBlending, DoLighting, AllowMovement, Backwards, Rotating: Boolean;
  ...
  public
    Picking: Boolean;
    Time: SmallInt;
    AllowTextureMode: boolean; // moved to public by Delfi so paths can detect if it is neccesary to turn back on textures after rendering
    ...
    function getdepthhitat(X, Y: LongWord): TVector3f;
...
var
  ...
  // DFIX: 
  RADAR_SIZE_X: single = 503;
  RADAR_SIZE_Y: single = 501;
  RADAR_Z: integer = -100;
...
// commented out to make gta3ers and lc-ers happy
//  gldisable(GL_ALPHA_TEST);
  ...
  for I := 0 to 7 do
    ...
    begin
      ...
      glBegin(GL_triangle_fan);
  		...
  		glVertex3f( (I - 4) * RADAR_SIZE_X, (J - 4) * RADAR_SIZE_Y, RADAR_Z);
      glTexCoord2f(1, 1);
  		glVertex3f( (I - 3) * RADAR_SIZE_X, (J - 4) * RADAR_SIZE_Y, RADAR_Z);
      glTexCoord2f(1, 0);
  		glVertex3f( (I - 3) * RADAR_SIZE_X, (J - 3) * RADAR_SIZE_Y, RADAR_Z);
      glTexCoord2f(0, 0);
  		glVertex3f( (I - 4) * RADAR_SIZE_X, (J - 3) * RADAR_SIZE_Y, RADAR_Z);
      glTexCoord2f(0, 1);
...
//    gluPerspective(45.0, Width/Height, 1.0, 8000.0);
    gluPerspective(proj, Width/Height, 1.0, 80000.0);
...
var
  ...
  Hit: PHit;
  SelType, SelFile, SelObject, SelNode: LongInt;
begin
  MinZ := LongWord(-1);
  CurrentPos := 0;
  SelFile := -1;
  ...
  idPath: begin
  ItemForm.JumpToFile(selfile, SECTION_PATH, selobject, selnode);
  end;
  idIDEPath:
  begin
    ShowMessage('Can''t do that with IDE paths yet');
  end;

  idZone: showmessage('can''t do that with zones yet');
  idCull: begin
  showmessage('can''t do that with cullzones yet');
//      ItemForm.SetToMode(FILE_IPL, SECTION_SEL_INST, SelFile, SelObject);
//      JumpToLocation(TIPLFile(PickFiles.Item[SelFile]).Inst.Item[SelObject].Pos);
  end;
  idoccl:
  begin
    ShowMessage('Can''t do that with OCCL yet');
  end;
...
begin
// testing..
//test:= getdepthhitat(x, y);
//showmessage(floattostr(ObjV[0]) + ', ' + floattostr(ObjV[1]) + ', ' + floattostr(ObjV[2]) );
  ...
  begin
    ...
    begin
      if (ssDouble in Shift) and (ssLeft in Shift) and not (ssCtrl in Shift) then
        ...
        MOUSEMODE_IPL, MOUSEMODE_MULT_INST:
        begin
...
begin
if PositionBox.Focused = false then
