  ...
  begin
    ...
    case SelSection of
      ...
      begin
        PanelInstSelect.Visible := False;
        DisplayListItems.Clear;
        with TIPLFile(GFiles.Item[SelFile]).occlu do
          if (Count > 0) then for I := 0 to Count - 1 do
            DisplayListItems.Add(Pointer(I));
      end;
      ...
      case SelSection of
        ...
        begin
          ...
          with TIPLFile(GFiles.Item[SelFile]).Cull.Item[LongWord(DisplayListItems.Items[Item.Index])] do
            Item.SubItems.Add(Format('(%f1.3, %f1.3, %f1.3)', [center[0], center[1], center[2]]));
        end;
        SECTION_ZONE:
        begin
          Item.Caption := IntToStr(LongWord(DisplayListItems.Items[Item.Index]));
        ...
        begin
          ...
          Item.SubItems.Add(Format('(%f1.3, %f1.3, %f1.3)', [
          TIPLFile(GFiles.Item[SelFile]).occlu.Item[item.index].Position[0],
          TIPLFile(GFiles.Item[SelFile]).occlu.Item[item.index].Position[1],
          TIPLFile(GFiles.Item[SelFile]).occlu.Item[item.index].Position[2]
          ]));
        end;
        SECTION_PATH:
        begin
  ...
  begin
    case SelSection of
      ...
      begin
        ...
        begin
          center := TIPLFile(GFiles.Item[SelFile]).Cull.Item[inCopy].center;

          start := TIPLFile(GFiles.Item[SelFile]).Cull.Item[inCopy].start;

          stop := TIPLFile(GFiles.Item[SelFile]).Cull.Item[inCopy].stop;

          flags := TIPLFile(GFiles.Item[SelFile]).Cull.Item[inCopy].flags;
