...
const
  ...
  PATH_COUNT = 11; // kcow used 12 here, thats not true, there are only 11 lines!!
  PATH_IPL_COUNT = 12;
...
begin
  ...
  roadblock := StrToIntDef(GetVal(11, Details), 0);
...
// kcow mistaken count and Rcount here causing path file grande corrtupte!
  ...
  begin
    Result := Result + #13#10#9 + Format('%d, %d, %d, %1.6g, %1.6g, %1.6g, %1.6g, %d, %d, %d, %d, %d',
     [NodeType, NodeConnect, U3, Pos[0], Pos[1], Pos[2], U7, LaneLeft, LaneRight, U10, roadblock, U12]);
  end;
...
function checkitem(x, y: integer): boolean;
begin
result:= not (
(Item[x].Item[y].Pos[0] = 0) and
(Item[x].Item[y].Pos[1] = 0) and
(Item[x].Item[y].Pos[2] = 0)
)
end;
...
if Main.MainGLView.Picking then glPushName(x);

for y:= 0 to Item[x].RCount-1 do begin

if Item[x].Item[y].NodeType <> 0 then
  if Item[x].Item[y].NodeConnect <> -1 then
  if checkitem(x, y) = true then begin

    // draw starting node
    if Main.MainGLView.Picking then glPushName(y); // the line is owned by first vertex, fixes error if user clicks onto the line.

    glBegin(GL_LINES);
    glVertex3f(Item[x].Item[y].Pos[0] * iplpathmp, Item[x].Item[y].Pos[1] * iplpathmp, Item[x].Item[y].Pos[2] * iplpathmp);
    glVertex3f(Item[x].Item[ Item[x].Item[y].NodeConnect ].Pos[0] * iplpathmp, Item[x].Item[ Item[x].Item[y].NodeConnect ].Pos[1] * iplpathmp, Item[x].Item[ Item[x].Item[y].NodeConnect ].Pos[2] * iplpathmp);
    glEnd;
    ...
    // draw ending node
    if Main.MainGLView.Picking then glPushName(Item[x].Item[y].NodeConnect);
    drawpathnode(Item[x].Item[ Item[x].Item[y].NodeConnect ].Pos, pathcubesize);
    if Main.MainGLView.Picking then glpopname;
...
begin
  ...
  U1 := StrToFloat(GetVal(10, Details));
...
begin
  ...
  Result := Format('%1.6g, %1.6g, %1.6g, %1.6g, %1.6g, %1.6g, %1.6g, %1.6g, %1.6g, %1.6g',
    [Pos1[0], Pos1[1], Pos1[2], Pos2[0], Pos2[1], Pos2[2], Pos3[0], Pos3[1], Pos3[2], U1]);
