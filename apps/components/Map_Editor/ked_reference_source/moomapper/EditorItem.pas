...
type
  TFormEditorItem = class(TForm)
    ...
    EditIPLPathItemWidth: TEdit;
    EditIPLPathItemConnect: TEdit;
    EditIPLPathItemU10: TEdit;
    EditIPLPathItemU11: TEdit;
    EditIPLPathItemU12: TEdit;
    EditIPLPathItemLanesLeft: TEdit;
    EditIPLPathItemLanesRight: TEdit;
    btnIPLPathShowInView: TBitBtn;
    ...
    EditIDEPathItemLanesLeft: TEdit;
    EditIDEPathItemLanesRight: TEdit;
    EditIDEPathItems: TListView;
    EditIDEPathModelName: TEdit;
    EditIDEPathModelID: TEdit;
    grpIDEPathType: TRadioGroup;
    Label53: TLabel;
    Label54: TLabel;
    ...
    Label44: TLabel;
    Label55: TLabel;
    procedure EditIDEPathItemLanesRightChange(Sender: TObject);
    procedure EditIDEPathItemLanesLeftChange(Sender: TObject);
    procedure EditIPLPathItemLanesRightChange(Sender: TObject);
    procedure EditIPLPathItemLanesLeftChange(Sender: TObject);
    procedure EditIDEPathModelNameChange(Sender: TObject);
    procedure EditIDEPathModelIDChange(Sender: TObject);
    procedure grpIDEPathTypeClick(Sender: TObject);
    ...
    procedure EditIDEPathItemsSelectItem(Sender: TObject; Item: TListItem;
      Selected: Boolean);
    procedure EditIDEPathItemTypeClick(Sender: TObject);
    ...
    procedure EditIDEPathItemPosChange(Sender: TObject);
    ...
    procedure EditIPLPathItemsSelectItem(Sender: TObject; Item: TListItem;
      Selected: Boolean);
    procedure EditIPLPathItemTypeClick(Sender: TObject);
    ...
    procedure EditIPLPathItemWidthChange(Sender: TObject);
    procedure EditIPLPathItemPosChange(Sender: TObject);
    ...
    procedure btnIPLPathShowInViewClick(Sender: TObject);
...
FullStr:= '';
  ...
  begin
    ...
    // Artem: More often than not file name is too long, so make a tooltip too
    FileIndexValue.Hint := FileIndexValue.Caption;
  ...
  case CurrentFileMode of
    ...
    begin
      case CurrentSection of
        ...
        SECTION_PATH:
        begin
          // Artem: Do IDE paths list book keeping
          with TIDEFile(GFiles.Item[CurrentFileIndex]).Path do
          begin
            i := GFiles.IDEPathsList.IndexOf(GetPathRep(
              Item[CurrentItemIndex].PathType,
              Item[CurrentItemIndex].ModelName));
            LoadItem(CurrentItemIndex, UndoStr);
            if i <> -1 then
            begin
              GFiles.IDEPathsList.Strings[i] :=
                GetPathRep(Item[CurrentItemIndex].PathType,
                Item[CurrentItemIndex].ModelName);
            end;
          end;
        end;
    ...
    begin
      case inSection of
        ...
        begin
          ...
          begin
            if (SameText(PathType, 'ped')) then
              grpIDEPathType.ItemIndex := 0
            else
              grpIDEPathType.ItemIndex := 1;

            EditIDEPathModelID.Text := IntToStr(ID);
            EditIDEPathModelName.Text := ModelName;
...
// Artem: extended to support IDE paths picking
        ...
        case CurrentSection of
          ...
          SECTION_path:
            with TIPLFile(GFiles.Item[CurrentFileIndex]).path.Item[CurrentItemIndex].Item[EditIPLPathItems.ItemIndex] do begin
              ...
              ManualChange := False;
              EditIPLPathItemPosX.Text := Format('%1.6g', [Pos[0] * iplpathmp]);
              EditIPLPathItemPosY.Text := Format('%1.6g', [Pos[1] * iplpathmp]);
              EditIPLPathItemPosZ.Text := Format('%1.6g', [Pos[2] * iplpathmp]);
              ManualChange := True;
            UpdateData(False, False);
          end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        EditIDEPathItemU7.Text := Format('%1.6g', [U7]);

        EditIDEPathItemPosX.Text := Format('%1.6g', [Pos[0] * iplpathmp]);
        EditIDEPathItemPosY.Text := Format('%1.6g', [Pos[1] * iplpathmp]);
        EditIDEPathItemPosZ.Text := Format('%1.6g', [Pos[2] * iplpathmp]);

        // Artem: Number of lanes can be up to 4,
        // makes sense to just use an EditBox
        EditIDEPathItemLanesLeft.Text := IntToStr(LaneLeft);
        EditIDEPathItemLanesRight.Text := IntToStr(LaneRight);
...
begin
  ...
  begin
    with TIDEFile(GFiles.Item[CurrentFileIndex]).Path.Item[CurrentItemIndex].Item[EditIDEPathItems.ItemIndex] do
      U7 := StrToFloatDef(EditIDEPathItemU7.Text, 0);
    UpdateData(False, False);
    EditIDEPathDrawlist;
  end;
end;

procedure TFormEditorItem.EditIDEPathItemPosChange(Sender: TObject);
begin
  if ManualChange then
  begin
    with TIDEFile(GFiles.Item[CurrentFileIndex]).Path.Item[CurrentItemIndex].Item[EditIDEPathItems.ItemIndex] do
    begin
      Pos[0] := StrToFloatDef(EditIDEPathItemPosX.Text, 0) / iplpathmp;
      Pos[1] := StrToFloatDef(EditIDEPathItemPosY.Text, 0) / iplpathmp;
      Pos[2] := StrToFloatDef(EditIDEPathItemPosZ.Text, 0) / iplpathmp;
    end;
...
procedure TFormEditorItem.grpIDEPathTypeClick(Sender: TObject);
var
  oldi, i: Integer;
  newid, ptype: string;
  InstList: TList;
begin
  if ManualChange then
  begin
    // Find the index for this path in the list of IDE paths
    with TIDEFile(GFiles.Item[CurrentFileIndex]).Path do
    begin
      oldi := GFiles.IDEPathsList.IndexOf(GetPathRep(Item[CurrentItemIndex].PathType,
        Item[CurrentItemIndex].ModelName));
      with Item[CurrentItemIndex] do
      begin
        if oldi <> -1 then
        begin
          // Find the index of the path with the same properties
          if grpIDEPathType.ItemIndex = 0 then
            ptype := 'ped'
          else
            ptype := 'car';
          newid := GetPathRep(ptype, ModelName);
          i := GFiles.IDEPathsList.IndexOf(newid);
          if i <> -1 then
          begin
            // If it already exists, disallow changes
            MessageDlg('The path with the same type already exists for this model',
              mtError, [mbOK], -1);
            ManualChange := False;
          ...
          begin
            ...
            PathType := ptype;
            UpdateData(False, False);
            GFiles.IDEPathsList[oldi] := newid;
            // Make sure that all instances are refreshed
            InstList := TList.Create;
            try
              GFiles.FindInstByModelName(EditIDEPathModelName.Text, InstList);
              for i := 0 to InstList.Count - 1 do
                PGTASInstObj(InstList[i])^.RefreshPath := True;
            finally
              InstList.Free;
            end;
          end;
        ...
        begin
          MessageDlg('Path not found', mtError, [mbOK], -1);
        end;
      end;
    end;
  end;
...
procedure TFormEditorItem.EditIDEPathModelIDChange(Sender: TObject);
begin
  if ManualChange then
  begin
    with TIDEFile(GFiles.Item[CurrentFileIndex]).Path.Item[CurrentItemIndex] do
      ID := StrToIntDef(EditIDEPathModelID.Text, 0);
    UpdateData(False, False);
  end;
end;

procedure TFormEditorItem.EditIDEPathModelNameChange(Sender: TObject);
var
  oldi, i: Integer;
  newid: string;
  InstList: TList;
  ...
  begin
    ...
    begin
      ...
      begin
        if oldi <> -1 then
        begin
          newid := GetPathRep(PathType, EditIDEPathModelName.Text);
          i := GFiles.IDEPathsList.IndexOf(newid);
          if i <> -1 then
          begin
            MessageDlg('The path with the same type already exists for this model',
            ...
            EditIDEPathModelName.Text := ModelName;
            ManualChange := True;
            EditIDEPathModelName.SetFocus;
          end
          else
          begin
            InstList := TList.Create;
            try
              // Make sure that old instances are refreshed
              GFiles.FindInstByModelName(ModelName, InstList);
              for i := 0 to InstList.Count - 1 do
                PGTASInstObj(InstList[i])^.RefreshPath := True;
              // Update
              ModelName := EditIDEPathModelName.Text;
              UpdateData(False, False);
              GFiles.IDEPathsList[oldi] := newid;
              // Make sure that new instances are refreshed
              InstList.Clear;
              GFiles.FindInstByModelName(ModelName, InstList);
              for i := 0 to InstList.Count - 1 do
                PGTASInstObj(InstList[i])^.RefreshPath := True;
...
procedure TFormEditorItem.EditIDEPathItemLanesLeftChange(Sender: TObject);
begin
  if ManualChange then
  begin
    with TIDEFile(GFiles.Item[CurrentFileIndex]).Path.Item[CurrentItemIndex].Item[EditIDEPathItems.ItemIndex] do
      LaneLeft := StrToIntDef(EditIDEPathItemLanesLeft.Text, 0);
    UpdateData(False, False);
    EditIDEPathDrawlist;
  end;
end;

procedure TFormEditorItem.EditIDEPathItemLanesRightChange(Sender: TObject);
begin
  if ManualChange then
  begin
    with TIDEFile(GFiles.Item[CurrentFileIndex]).Path.Item[CurrentItemIndex].Item[EditIDEPathItems.ItemIndex] do
      LaneRight := StrToIntDef(EditIDEPathItemLanesRight.Text, 0);
    UpdateData(False, False);
    EditIDEPathDrawlist;
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      LItem.SubItems.Add(Format('(%1.6g, %1.6g, %1.6g)',
        [Item[I].Pos[0] * iplpathmp, Item[I].Pos[1] * iplpathmp,
        Item[I].Pos[2] * iplpathmp]));
    end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        EditIPLPathItemWidth.Text := Format('%1.6g', [Width]);

        EditIPLPathItemPosX.Text := Format('%1.6g', [Pos[0] * iplpathmp]);
        EditIPLPathItemPosY.Text := Format('%1.6g', [Pos[1] * iplpathmp]);
        EditIPLPathItemPosZ.Text := Format('%1.6g', [Pos[2] * iplpathmp]);

        EditIPLPathItemLanesLeft.Text := IntToStr(LaneLeft);
        EditIPLPathItemLanesRight.Text := IntToStr(LaneRight);

        EditIPLPathItemU10.Text := IntToStr(U10);
        EditIPLPathItemU11.Text := IntToStr(Flags);
        EditIPLPathItemU12.Text := Format('%1.6g', [U12]);
      end;
      ManualChange := True;
    end;
  end;
...
procedure TFormEditorItem.EditIPLPathItemTypeClick(Sender: TObject);
begin
  if ManualChange then
  begin
...
begin
  if ManualChange then
  begin
    TIPLFile(GFiles.Item[CurrentFileIndex]).Path.Item[CurrentItemIndex].Item[EditIPLPathItems.ItemIndex].Flags := StrToIntDef(EditIPLPathItemU11.Text, 0);
    UpdateData(False, False);
    EditIPLPathDrawlist;
  end;
end;
...
begin
  ...
  begin
    with TIPLFile(GFiles.Item[CurrentFileIndex]).Path.Item[CurrentItemIndex].Item[EditIPLPathItems.ItemIndex] do
      U12 := StrToFloatDef(EditIPLPathItemU12.Text, 0);
    UpdateData(False, False);
    EditIPLPathDrawlist;
  end;
end;

procedure TFormEditorItem.EditIPLPathItemWidthChange(Sender: TObject);
begin
  if ManualChange then
  begin
    TIPLFile(GFiles.Item[CurrentFileIndex]).Path.Item[CurrentItemIndex].Item[EditIPLPathItems.ItemIndex].Width := StrToFloatDef(EditIPLPathItemWidth.Text, 0);
    UpdateData(False, False);
    EditIPLPathDrawlist;
  end;
end;

procedure TFormEditorItem.EditIPLPathItemPosChange(Sender: TObject);
begin
  if ManualChange then
  begin
    with TIPLFile(GFiles.Item[CurrentFileIndex]).Path.Item[CurrentItemIndex].Item[EditIPLPathItems.ItemIndex] do begin
    Pos[0] := StrToFloatDef(EditIPLPathItemPosX.Text, 0) / iplpathmp;
    Pos[1] := StrToFloatDef(EditIPLPathItemPosY.Text, 0) / iplpathmp;
    Pos[2] := StrToFloatDef(EditIPLPathItemPosZ.Text, 0) / iplpathmp;
    end;
...
procedure TFormEditorItem.EditIPLPathItemLanesLeftChange(Sender: TObject);
begin
  if ManualChange then
  begin
    with TIPLFile(GFiles.Item[CurrentFileIndex]).Path.Item[CurrentItemIndex].Item[EditIPLPathItems.ItemIndex] do
      LaneLeft := StrToIntDef(EditIPLPathItemLanesLeft.Text, 0);
    UpdateData(False, False);
    EditIPLPathDrawlist;
  end;
end;

procedure TFormEditorItem.EditIPLPathItemLanesRightChange(Sender: TObject);
begin
  if ManualChange then
  begin
    with TIPLFile(GFiles.Item[CurrentFileIndex]).Path.Item[CurrentItemIndex].Item[EditIPLPathItems.ItemIndex] do
      LaneRight := StrToIntDef(EditIPLPathItemLanesRight.Text, 0);
    UpdateData(False, False);
    EditIPLPathDrawlist;
  end;
end;
...
procedure TFormEditorItem.btnIPLPathShowInViewClick(Sender: TObject);
var
tmp: TVector3f;
begin
