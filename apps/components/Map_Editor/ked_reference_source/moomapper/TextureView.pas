...
uses
  ...
  Dialogs, ExtCtrls, ComCtrls, StdCtrls, ActnList, OpenGl, RequiredTypes,
  Buttons; // geometry
...
begin
  ...
  // background color
  glClearColor(GetRValue(ColorBox1.selected) / 255,
               GetGValue(ColorBox1.selected) / 255,
               GetBValue(ColorBox1.selected) / 255,
               0.0);
...
procedure TFormTextureView.BitBtn1Click(Sender: TObject);
begin
Image1.Picture.Bitmap.Width:= Main.GArchive.GTxd[TextureNum].Image[ImageNum].Width;
Image1.Picture.Bitmap.height:= Main.GArchive.GTxd[TextureNum].Image[ImageNum].height;

case Main.GArchive.GTxd[TextureNum].Image[ImageNum].Depth of
32: Image1.Picture.Bitmap.PixelFormat:= pf32bit;
16: Image1.Picture.Bitmap.PixelFormat:= pf32bit; // check dxtš
8: Image1.Picture.Bitmap.PixelFormat:= pf8bit;
end;
