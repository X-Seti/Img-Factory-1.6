...
class DP5Workshop(ColorPalPresetsMixin, QWidget):
    ...
    # ── Tool icon generator (inline SVG, DPaint5-faithful shapes) ─────────────

    @staticmethod
    def _make_tool_icon(tool_id: str, size: int = 24, color: str = '#ffffff') -> 'QIcon':
        """Generate an inline SVG QIcon for each DP5 tool."""
        c = color or '#ffffff'
        s = size
        h = s / 2  # half

        svgs = {
            TOOL_PENCIL: f'''<svg viewBox="0 0 {s} {s}" xmlns="http://www.w3.org/2000/svg">
              <line x1="{s*0.1}" y1="{s*0.9}" x2="{s*0.8}" y2="{s*0.2}"
                    stroke="{c}" stroke-width="2" stroke-linecap="round"/>
              <polygon points="{s*0.8},{s*0.2} {s*0.95},{s*0.05} {s*0.85},{s*0.3}"
                       fill="{c}"/>
              <line x1="{s*0.05}" y1="{s*0.95}" x2="{s*0.15}" y2="{s*0.85}"
                    stroke="{c}" stroke-width="2"/>
            </svg>''',

            TOOL_ERASER: f'''<svg viewBox="0 0 {s} {s}" xmlns="http://www.w3.org/2000/svg">
              <rect x="{s*0.1}" y="{s*0.35}" width="{s*0.8}" height="{s*0.4}"
                    rx="2" fill="none" stroke="{c}" stroke-width="1.8"/>
              <line x1="{s*0.1}" y1="{s*0.75}" x2="{s*0.9}" y2="{s*0.75}"
                    stroke="{c}" stroke-width="1.5"/>
              <line x1="{s*0.4}" y1="{s*0.35}" x2="{s*0.4}" y2="{s*0.75}"
                    stroke="{c}" stroke-width="1.2" opacity="0.5"/>
            </svg>''',

            TOOL_FILL: f'''<svg viewBox="0 0 {s} {s}" xmlns="http://www.w3.org/2000/svg">
              <polygon points="{s*0.15},{s*0.85} {s*0.5},{s*0.1} {s*0.85},{s*0.85}"
                       fill="{c}" opacity="0.85"/>
              <circle cx="{s*0.75}" cy="{s*0.72}" r="{s*0.12}"
                      fill="none" stroke="{c}" stroke-width="1.5"/>
              <line x1="{s*0.75}" y1="{s*0.84}" x2="{s*0.75}" y2="{s*0.95}"
                    stroke="{c}" stroke-width="2" stroke-linecap="round"/>
            </svg>''',

            TOOL_PICKER: f'''<svg viewBox="0 0 {s} {s}" xmlns="http://www.w3.org/2000/svg">
              <circle cx="{h}" cy="{h}" r="{s*0.38}"
                      fill="none" stroke="{c}" stroke-width="1.8"/>
              <line x1="{h}" y1="{s*0.08}" x2="{h}" y2="{s*0.92}"
                    stroke="{c}" stroke-width="1.4"/>
              <line x1="{s*0.08}" y1="{h}" x2="{s*0.92}" y2="{h}"
                    stroke="{c}" stroke-width="1.4"/>
              <circle cx="{h}" cy="{h}" r="{s*0.08}" fill="{c}"/>
            </svg>''',

            TOOL_LINE: f'''<svg viewBox="0 0 {s} {s}" xmlns="http://www.w3.org/2000/svg">
              <line x1="{s*0.12}" y1="{s*0.88}" x2="{s*0.88}" y2="{s*0.12}"
                    stroke="{c}" stroke-width="2.2" stroke-linecap="round"/>
              <circle cx="{s*0.12}" cy="{s*0.88}" r="{s*0.07}" fill="{c}"/>
              <circle cx="{s*0.88}" cy="{s*0.12}" r="{s*0.07}" fill="{c}"/>
            </svg>''',

            TOOL_RECT: f'''<svg viewBox="0 0 {s} {s}" xmlns="http://www.w3.org/2000/svg">
              <rect x="{s*0.12}" y="{s*0.22}" width="{s*0.76}" height="{s*0.56}"
                    fill="none" stroke="{c}" stroke-width="2" rx="1"/>
            </svg>''',

            TOOL_CIRCLE: f'''<svg viewBox="0 0 {s} {s}" xmlns="http://www.w3.org/2000/svg">
              <ellipse cx="{h}" cy="{h}" rx="{s*0.38}" ry="{s*0.3}"
                       fill="none" stroke="{c}" stroke-width="2"/>
            </svg>''',

            TOOL_SPRAY: f'''<svg viewBox="0 0 {s} {s}" xmlns="http://www.w3.org/2000/svg">
              <circle cx="{s*0.25}" cy="{s*0.3}"  r="{s*0.06}" fill="{c}"/>
              <circle cx="{s*0.55}" cy="{s*0.2}"  r="{s*0.05}" fill="{c}" opacity="0.8"/>
              <circle cx="{s*0.7}"  cy="{s*0.4}"  r="{s*0.06}" fill="{c}"/>
              <circle cx="{s*0.35}" cy="{s*0.55}" r="{s*0.05}" fill="{c}" opacity="0.7"/>
              <circle cx="{s*0.6}"  cy="{s*0.6}"  r="{s*0.07}" fill="{c}"/>
              <circle cx="{s*0.2}"  cy="{s*0.7}"  r="{s*0.05}" fill="{c}" opacity="0.6"/>
              <circle cx="{s*0.75}" cy="{s*0.72}" r="{s*0.05}" fill="{c}" opacity="0.8"/>
              <circle cx="{s*0.45}" cy="{s*0.78}" r="{s*0.06}" fill="{c}" opacity="0.7"/>
            </svg>''',
        }

        svg_str = svgs.get(tool_id, '')
        if not svg_str:
            return QIcon()
    ...
    def _create_right_panel(self):
        ...
        # Tool definitions: (tool_id, tooltip, row, col)
        tools = [
            (TOOL_PENCIL, 'Pencil (P)',        0, 0),
            (TOOL_ERASER, 'Eraser (E)',        0, 1),
            (TOOL_FILL,   'Flood Fill (F)',     1, 0),
            (TOOL_PICKER, 'Colour Picker (K)',  1, 1),
            (TOOL_LINE,   'Line (L)',           2, 0),
            (TOOL_RECT,   'Rect (R)',           2, 1),
            (TOOL_CIRCLE, 'Circle (C)',         3, 0),
            (TOOL_SPRAY,  'Spray (S)',          3, 1),
        ]

        icon_color = self._get_icon_color()
        self._tool_btns = {}
        for tool_id, tip, row, col in tools:
            btn = QPushButton()
            btn.setFixedSize(36, 36)
            btn.setCheckable(True)
            btn.setToolTip(tip)
            ico = self._make_tool_icon(tool_id, 24, icon_color)
            if ico and not ico.isNull():
                ...
                fallbacks = {
                    ...
                    TOOL_PICKER: '⊕', TOOL_LINE:   '╱', TOOL_RECT:  '▭',
                    TOOL_CIRCLE: '○', TOOL_SPRAY:  '·:',
                }
                btn.setText(fallbacks.get(tool_id, '?'))
        ...
        if ICONS_AVAILABLE:
            ico = SVGIconFactory.undo_icon(24, icon_color)
            if not ico.isNull():
                undo_btn.setIcon(ico)
                undo_btn.setIconSize(QSize(24, 24))
            else:
                undo_btn.setText("UN\nDO"); undo_btn.setFont(QFont("Arial", 7))
        else:
            undo_btn.setText("UN\nDO"); undo_btn.setFont(QFont("Arial", 7))
