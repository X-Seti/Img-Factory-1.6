#this belongs in gui/gui_settings.py - Version: 5
# X-Seti - Sep 22 2026 - IMG Factory 1.6 - Consolidated Settings Dialog
...
# _choose_font
# _create_buttons_tab
# _create_file_types_tab
# _create_fonts_tab
# _create_gui_tab
# _create_ui
# _load_current_settings
# _load_file_types
# _remove_file_type_row
# _reset_to_defaults
# _save_and_close
# _save_file_types
# _save_settings
...
class SettingsDialog(QDialog): #vers 4
    ...
    def _create_ui(self): #vers 4
        ...
        self.tab_widget.addTab(self._create_file_types_tab(), "File Types")
    ...
    def _create_file_types_tab(self): #vers 1
        """Create File Types tab - extension to workshop/app associations"""
        widget = QWidget()
        layout = QVBoxLayout(widget)

        layout.addWidget(QLabel("Extension → workshop or external app:"))

        self.file_types_table = QTableWidget(0, 2)
        self.file_types_table.setHorizontalHeaderLabels(["Extension", "Handler"])
        self.file_types_table.horizontalHeader().setSectionResizeMode(1, QHeaderView.ResizeMode.Stretch)
        layout.addWidget(self.file_types_table)

        btn_layout = QHBoxLayout()
        add_btn = QPushButton("Add")
        add_btn.clicked.connect(self._add_file_type_row)
        btn_layout.addWidget(add_btn)
        remove_btn = QPushButton("Remove")
        remove_btn.clicked.connect(self._remove_file_type_row)
        btn_layout.addWidget(remove_btn)
        btn_layout.addStretch()
        save_btn = QPushButton("Save File Types")
        save_btn.clicked.connect(self._save_file_types)
    ...
    def _load_file_types(self): #vers 1
        """Populate the file types table from file_associations.py"""
        from apps.methods.file_associations import get_associations
        assoc = get_associations()
        self.file_types_table.setRowCount(0)
        for ext, handler in sorted(assoc.items()):
            self._add_file_type_row(ext, handler)

    def _add_file_type_row(self, ext="", handler=""): #vers 1
        """Add one extension/handler row to the table"""
        row = self.file_types_table.rowCount()
        self.file_types_table.insertRow(row)
        self.file_types_table.setItem(row, 0, QTableWidgetItem(ext))
        self.file_types_table.setItem(row, 1, QTableWidgetItem(handler))

    def _remove_file_type_row(self): #vers 1
        """Remove the selected row from the file types table"""
        row = self.file_types_table.currentRow()
        if row >= 0:
            self.file_types_table.removeRow(row)

    def _save_file_types(self): #vers 1
        """Save the file types table to file_associations.json"""
        from apps.methods.file_associations import save_associations
        assoc = {}
        for row in range(self.file_types_table.rowCount()):
            ext_item = self.file_types_table.item(row, 0)
            handler_item = self.file_types_table.item(row, 1)
            ext = ext_item.text().strip().lstrip('.').lower() if ext_item else ""
            handler = handler_item.text().strip() if handler_item else ""
            if ext and handler:
                assoc[ext] = handler
        save_associations(assoc)
        parent = self.parent()
        if parent and hasattr(parent, 'log_message'):
            parent.log_message("File type associations saved")
