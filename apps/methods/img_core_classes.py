...
def _detect_v1_or_v1_5(dir_path: str, img_path: str):
    """Detect whether a DIR/IMG pair is standard V1 or extended V1.5.
    V1.5 indicators: IMG file > 2GB, or any entry name longer than 24 chars (no null in name field)."""
    try:
        import os as _os
        # Size check - V1 is capped at 2GB
        if _os.path.getsize(img_path) > 2 * 1024 * 1024 * 1024:
            return 'V1_5'
        # Check entry names in DIR file for extended names
        with open(dir_path, 'rb') as f:
            while True:
                entry = f.read(32)
                if len(entry) < 32:
                    break
                name_bytes = entry[8:32]
...
class IMGVersion(Enum):
    ...
    VERSION_1_5 = 15 # Extended DIR/IMG pair - up to 4GB, long filenames
            ...
            if self.file_path.lower().endswith('.img'):
                try:
                    with open(self.file_path, 'rb') as f:
                        ...
                        # Could be Version 1/1.5 IMG without DIR
