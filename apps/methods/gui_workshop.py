...
class _ToolbarMixin:
    ...
    def _create_toolbar(self):
        ...
        # - visible in a real Water Workshop screenshot, docked inside
        # IMG Factory, still showing its own full internal titlebar
        # (Menu/Settings/title text/undo/info/settings/D) duplicating
        # the outer tab's own title). Shared GUIWorkshop base class,
        # so this fixes every "*_Workshop" tool at once, not just
        # Water Workshop - the standalone-only window controls
        # (minimize/maximize/close) already only ever showed in
        # standalone mode; hiding the whole bar rather than just those
        # is the natural extension of that same, already-established
        # distinction, not a new one. self.dock_btn (the "D" button
        # this same toolbar builds above) is hidden along with
        # everything else here - undocking while already docked needs
        # its own, separate real access point elsewhere if that's
        # still wanted; simply always showing the whole titlebar again
        # just to keep this one button reachable would bring back
        # exactly the duplication being fixed.
