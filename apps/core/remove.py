...
def _remove_entries_with_tracking(file_object, entries_to_remove: List, main_window) -> bool: #vers 2
    ...
    if removed_count > 0:
        ...
        # Sync pin file - remove deleted entry keys
        img_path = getattr(file_object, 'file_path', None)
        if img_path:
            removed_names = [getattr(e, 'name', '') for e in entries_to_remove
                             if not getattr(e, 'is_pinned', False)]
            from apps.core.undo_system import pin_file_sync_remove
            pin_file_sync_remove(img_path, removed_names)
        if hasattr(main_window, 'log_message'):
